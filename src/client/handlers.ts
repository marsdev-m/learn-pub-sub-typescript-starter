...
export function handlerWar(
  gs: GameState,
  ch: ConfirmChannel,
): (rs: RecognitionOfWar) => Promise<AckType> {
      ...
      if (warResult.result === WarOutcome.NotInvolved) {
        ...
        try {
          publishGameLog(
            ch,
            gs.getUsername(),
            `${warResult.winner} won a war against ${warResult.loser}`,
          );
          return AckType.Ack;
        } catch (e) {
          return AckType.NackRequeue;
        ...
        try {
          publishGameLog(
            ch,
            gs.getUsername(),
            `A war between ${warResult.attacker} and ${warResult.defender} resulted in a draw`
          );
          return AckType.Ack;
        } catch (e) {
          return AckType.NackRequeue;
